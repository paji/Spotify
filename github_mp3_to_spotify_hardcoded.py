...
class GitHubMP3Fetcher:
    ...
    def __init__(self, username, repo, token=None, branch='main', path=''):
        """
        初期化メソッド
        ...
        Args:
            ...
            repo (str): リポジトリ名
            token (str, optional): GitHub APIトークン（レート制限緩和用）
...
def fetch_mp3_files_from_github(args):
    ...
    if not token:
        token = input("GitHub APIトークンを入力してください（オプション、Enterでスキップ）: ").strip() or None
    if branch == 'main':
    ...
    fetcher = GitHubMP3Fetcher(username, repo, token, branch, path)
...
def main():
    ...
    fetch_parser.add_argument('--token', '-t', help='GitHub APIトークン（オプション）')
    ...
    all_parser.add_argument('--token', '-t', help='GitHub APIトークン（オプション）')
