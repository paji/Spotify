...
def update_rss_for_github_pages():
    ...
    try:
        ...
        for item in root.findall('.//item'):
            # タイトルなどのテキスト要素に対して適切なエンコーディングを確保
            for title in item.findall('.//title'):
                if title.text:
                    # タイトルのテキストをデコードしてエスケープされていないことを確認
            ...
            for enclosure in item.findall('.//enclosure'):
                ...
                new_url = f"{github_pages_url}/{encoded_filename}"
...
"""
    ...
    for file in mp3_files:
        ...
        if match:
            ...
            title_display = html.escape(title.replace('"', ''))
            # ファイル名をURLエンコード
...
"""
            ...
            # 日付パターンに一致しないファイルの場合はそのまま表示
            title_display = html.escape(file)
            file_encoded = urllib.parse.quote(file)
