...
class GitHubMP3Fetcher:
    ...
    def fetch_mp3_files(self):
        ...
        # レポートをYAMLファイルに保存
        with open("fetch_report.yml", "w", encoding="utf-8") as f:
            yaml.dump(self.report_data, f, default_flow_style=False, allow_unicode=True)
...
class RSSGenerator:
    ...
    def generate_rss(self):
        ...
        # レポートをYAMLファイルに保存
        with open("rss_report.yml", "w", encoding="utf-8") as f:
            yaml.dump(self.report_data, f, default_flow_style=False, allow_unicode=True)
...
def main():
    ...
    # 全体のレポートをYAMLファイルに保存
    with open("execution_report.yml", "w", encoding="utf-8") as f:
        yaml.dump(report_data, f, default_flow_style=False, allow_unicode=True)
    
    print(f"詳細なレポートを execution_report.yml に保存しました。")
