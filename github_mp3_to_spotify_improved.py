...
class GitHubMP3Fetcher:
    ...
    def fetch_mp3_files(self):
        ...
        print(f"取得したMP3ファイル数: {len(mp3_files)}")
        for mp3 in mp3_files:
            print(f"  - {mp3['filename']} ({mp3['url']})")
        return mp3_files
    ...
    def _fetch_files_recursive(self, current_path, mp3_files):
        ...
        print(f"GitHubコンテンツAPIにアクセス中: {url}")
        try:
        ...
        for item in items:
            if item["type"] == "file" and item["name"].lower().endswith(".mp3"):
                ...
                print(f"MP3ファイル情報を追加: {file_name}")
                
            elif item["type"] == "dir":
                # ディレクトリの場合は再帰的に処理
                dir_path = item["path"]
                print(f"ディレクトリを検索: {dir_path}")
...
class RSSGenerator:
    ...
    def __init__(self, mp3_files, config_file=None):
        ...
        print(f"RSSGenerator: 受け取ったMP3ファイル数: {len(mp3_files)}")
    ...
    def generate_rss(self):
        ...
        print(f"RSSに追加したアイテム数: {added_count}")
