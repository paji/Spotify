...
class GitHubMP3Fetcher:
    ...
    def _fetch_files_recursive(self, current_path, mp3_files):
        ...
        url = f"{self.base_url}/{current_path}?ref={self.commit_hash}"
        if current_path:
            url = f"{self.base_url}/{current_path}?ref={self.commit_hash}"
        else:
        ...
        try:
            ...
            for item in items:
                if item["type"] == "file" and item["name"].lower().endswith(".mp3"):
                    ...
                    # 最終更新日を取得
                    file_sha = item["sha"]
                    commit_url = f"https://api.github.com/repos/{self.username}/{self.repo}/commits?path={quote(file_path)}&sha={self.commit_hash}"
