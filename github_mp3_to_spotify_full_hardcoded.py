...
すべてのパラメータ（ポッドキャスト情報、GitHub APIトークン、ブランチ名など）は
ハードコーディングされています。
"""
...
class GitHubMP3Fetcher:
    ...
    def __init__(self, username, repo, path=''):
        ...
        self.raw_base_url = f"https://raw.githubusercontent.com/{username}/{repo}/{self.branch}"
...
def fetch_mp3_files_from_github(args):
    ...
    # ユーザー名とリポジトリ名が指定されていない場合は入力を求める
    # ただし、GitHub Actionsでの実行時にはこれらのプロンプトは表示されない
    if not username:
        username = input("GitHubユーザー名を入力してください: ")
    if not repo:
        repo = input("リポジトリ名を入力してください: ")
    
    # パスが指定されていない場合は空文字列を使用
    if not path:
        path = ""
    
    # MP3ファイルを取得
    fetcher = GitHubMP3Fetcher(username, repo, path)
